...
app.post("/api/shelf/add", async (req, res) => {
  try {
    const { googleID, isbn, title, author} = req.body;

    if (typeof googleID !== "number" || typeof isbn !== "number") {
      return res.status(400).json({
        success: false,
        error: "Missing googleID or isbn",
        message: "missing fields!"
      });
    }

    const book = await db.findCreateBook(isbn, title, author, "testing service", "test image")
